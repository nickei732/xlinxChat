...
class SelectMembersViewModel extends BaseViewModel {
  ...
  void selectUserClick(CategoryData user) async {
    ...
    if (isGroup) {
      ...
      setBusy(true);
      String chatId = '';
      if (user.customerFname.hashCode <= prefs.getString("UserId").hashCode) {
        chatId = '${user.customerId}-${prefs.getString("UserId")}';
      } else {
        chatId = '${prefs.getString("UserId")}-${user.customerId}';
      }
      ...
      SendNotificationModel sendNotificationModel = SendNotificationModel(
        fcmToken: user.customerId.toString(),
        roomId: chatId,
      ...
      (user.customerId.toString() !=  prefs.getString("token")) ? messagingService.sendNotification(sendNotificationModel) : null;
